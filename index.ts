...
export default {
    async fetch(request: Request, env: Environment): Promise<Response> {
        ...
        if (["/d1/users", "/d1/users/limited", "/d1/users/counts", "/d1/outputs", "/d1/outputs/limited", "/d1/outputs/counts"].includes(url.pathname) && ["GET", "POST"].includes(request.method)) return D1Handler(request, env);
