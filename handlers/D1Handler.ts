...
export default async (request: Request, env: Environment): Promise<Response> => {
    ...
    if (url.pathname === "/d1/users") {
        if (request.method === "POST") {
            ...
            const { results } = await env.DB.prepare(isAdmin ? "SELECT * FROM users" : "SELECT total_stats_sent FROM users ORDER BY total_stats_sent DESC").all();
            return json(results);
        }
    }
    ...
    if (url.pathname === "/d1/outputs") {
        if (request.method === "POST") {
            ...
            const { results } = await env.DB.prepare(isAdmin ? "SELECT * FROM outputs" : "SELECT game, segment, language, date FROM outputs").all();
            return json(results);
        }
    }
